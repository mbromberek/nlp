...
for token in doc:
    if token.text == 'fruit':
        fruit_adjectives = fruit_adjectives + \
            [modifier.text for modifier in token.lefts if modifier.pos_ == 'ADJ']
        fruit_origins = fruit_origins + \
            [doc[modifier.i + 1].text for modifier in token.rights \
            if modifier.text == 'from' and doc[modifier.i+1].ent_type != 0]
